...
class RTTMainWindow(QMainWindow):
    def __init__(self):
        ...
        logger.info("🔧 设置RTT控件状态...")
        self._set_rtt_controls_enabled(False)
        logger.info("✅ RTT控件状态设置完成")
        ...
        # 立即创建连接对话框以便加载配置
        logger.info("🔧 开始创建连接对话框...")
        self.connection_dialog = ConnectionDialog(self)
        logger.info("✅ 连接对话框创建完成")
        # 连接成功信号
    ...
    def _create_menu_bar(self):
        ...
        # 从配置恢复默认方式
        try:
            # 使用全局配置管理器，因为connection_dialog此时还未创建
            default_method = config_manager.get_restart_method()
        except Exception:
            default_method = 'SFR'
...
class ConnectionDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        try:
            logger.info("🔧 初始化JLink对象...")
            self.jlink = pylink.JLink()
            logger.info("✅ JLink对象初始化成功")
        except Exception as e:
            logger.error(f'Find jlink dll failed: {e}', exc_info=True)
            self.jlink = None  # 设为None继续运行
            logger.warning("⚠️ JLink初始化失败，程序将在无JLink模式下运行")
        ...
        # 检测可用的JLINK设备
        logger.info("🔧 开始检测JLINK设备...")
        self._detect_jlink_devices()
        logger.info("✅ JLINK设备检测完成")
        ...
        try:
            ...
            if self.jlink._library._path is not None and not self._device_database_exists():
                ...
                if self.jlink._library._windows or self.jlink._library._cygwin:
                    ...
                    try:
                        process = subprocess.run(cmd, check=False, startupinfo=startupinfo, 
                                               creationflags=subprocess.CREATE_NEW_CONSOLE, 
                                               timeout=5)  # 5秒超时
                    except subprocess.TimeoutExpired:
                        logger.warning("JLink command export timeout, continuing...")

...
exe = EXE(
    ...
    version_file='version_info_v2_2.txt',  # 版本信息
...
def build_onefile():
    """构建单文件版本"""
    print("XexunRTT v2.2 单文件版本构建工具")
    print("=" * 60)
    print("目标: 生成单个EXE文件，便于分发")
    print("特点: 所有依赖打包到一个文件中")
    print("新增: 筛选TAB正则表达式功能")
    print()
    ...
    # 创建spec文件
    spec_filename = "XexunRTT_onefile_v2_2.spec"
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if exe_path.exists():
                ...
                print("\n🚀 使用方法:")
                print("1. 复制XexunRTT_v2.2.exe到任意目录")
...
def create_portable_package():
    ...
    # 创建便携版目录
    portable_dir = Path("dist") / "XexunRTT_v2.2_Portable"
    portable_dir.mkdir(exist_ok=True)
    
    # 复制EXE文件
    portable_exe = portable_dir / "XexunRTT_v2.2.exe"
    shutil.copy2(exe_path, portable_exe)
    
    # 创建说明文件
    readme_content = """# XexunRTT v2.2 便携版

## 🚀 快速开始
1. 双击 `XexunRTT_v2.2.exe` 即可运行
2. 首次运行会自动创建配置文件
3. 无需安装任何其他软件

## 📁 文件说明
- `XexunRTT_v2.2.exe`: 主程序（单文件版本）
...
def create_release_notes():
    """创建发布说明"""
    release_notes = """# XexunRTT v2.2 发布说明

## 🎯 版本亮点
v2.2 新增正则表达式筛选功能，修复TAB切换重复数据问题，提升用户体验。
...
"""
    ...
    release_file = Path("RELEASE_NOTES_v2.2.md")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("\n" + "=" * 60)
            print("🎉 XexunRTT v2.2 单文件版本构建完成!")
            print("\n📁 输出文件:")
            print("- dist/XexunRTT_v2.2.exe (单文件版本)")
            print("- dist/XexunRTT_v2.2_Portable/ (便携版包)")
            print("- RELEASE_NOTES_v2.2.md (发布说明)")

...
class rtt_to_serial():
    def __init__(self, worker, jlink, connect_inf='USB', connect_para=None, device=None, port=None, baudrate=115200, interface=pylink.enums.JLinkInterfaces.SWD, speed=12000, reset=False, log_split=True, window_id=None, jlink_index=None, rtt_cb_mode='auto', rtt_address='', rtt_search_range='', skip_rtt_block_detection=False):
        ...
        self._auto_reset_cooldown_sec = 2.0
    ...
    def _is_jlink_dll_not_open(self, err) -> bool:
        """判断是否为 J-Link DLL 未打开错误。"""
        try:
            return "dll is not open" in str(err).lower()
        except Exception:
            return False

    def _recover_from_dll_not_open(self, context: str = "") -> bool:
        """在运行中恢复 J-Link DLL 未打开状态（无需重启APP）。"""
        try:
            self._log_to_gui(
                QCoreApplication.translate(
                    "rtt2uart",
                    "Detected J-Link DLL not open (%s), trying in-app recovery..."
                ) % (context or "runtime")
            )
            ok = self._try_auto_reset_connection(f"dll_not_open:{context}")
            if ok:
                self._log_to_gui(QCoreApplication.translate("rtt2uart", "J-Link DLL recovery succeeded"))
            else:
                self._log_to_gui(QCoreApplication.translate("rtt2uart", "J-Link DLL recovery failed"))
            return bool(ok)
        except Exception as e:
            logger.warning(f"Recover from dll-not-open failed: {e}")
            return False
    ...
    def _request_ui_disconnect(self):
        """请求当前 ConnectionDialog 清理旧会话，避免 already open 卡死。"""
        try:
            parent = getattr(self.worker, "parent", None)
            if parent and hasattr(parent, "reset_after_jlink_recovery"):
                from PySide6.QtCore import QMetaObject, Qt, QThread
                parent_thread = parent.thread() if hasattr(parent, "thread") else None
                current_thread = QThread.currentThread()
                if parent_thread is not None and current_thread == parent_thread:
                    logger.warning("[UI-DISCONNECT] running synchronous cleanup on current dialog")
                    parent.reset_after_jlink_recovery()
                else:
                    QMetaObject.invokeMethod(
                        parent,
                        "reset_after_jlink_recovery",
                        Qt.BlockingQueuedConnection,
                    )
                try:
                    self._log_to_gui(QCoreApplication.translate("rtt2uart", "Requesting current session cleanup"))
                except Exception:
                    pass
                return True
        except Exception as e:
    ...
    def _sync_connection_dialog_references(self):
        """同步重建后的 JLink/RTT 对象到当前 ConnectionDialog。"""
        try:
            parent = getattr(self.worker, "parent", None)
            if parent is None:
                return
            parent.jlink = self.jlink
            parent.rtt2uart = self
        except Exception as e:
            logger.debug(f"[SYNC-DIALOG] failed: {e}")
    ...
    def _try_auto_reset_connection(self, reason: str = "") -> bool:
        """尝试自动重置JLink连接（防重入、冷却）。返回是否已处理/成功。"""
        try:
            now = time.time()
            # 已有重置在进行，视为已处理
            if self._auto_reset_in_progress:
                logger.info(f"[AUTO-RESET] Already in progress, skip duplicate: {reason}")
                return True
            # 冷却期内不重复触发
            if (now - self._auto_reset_last_ts) < self._auto_reset_cooldown_sec:
                logger.info(f"[AUTO-RESET] Cooldown active, skip: {reason}")
                return True
            ...
            with self._auto_reset_lock:
                ...
                now = time.time()
                if self._auto_reset_in_progress:
                    logger.info(f"[AUTO-RESET] Already in progress (locked), skip: {reason}")
                    return True
                if (now - self._auto_reset_last_ts) < self._auto_reset_cooldown_sec:
                    logger.info(f"[AUTO-RESET] Cooldown active (locked), skip: {reason}")
                    return True
                self._auto_reset_in_progress = True

            # 重置期间暂停读取，避免噪音
            self._suspend_rtt_reads = True
            ok = self._auto_reset_jlink_connection()
            return bool(ok)
        except Exception as e:
            logger.error(f"[AUTO-RESET] Exception during auto reset: {e}")
            return False
        finally:
            # 释放状态
            try:
            ...
            try:
                if self._auto_reset_in_progress:
                    self._auto_reset_last_ts = time.time()
                    self._auto_reset_in_progress = False
            except Exception:
                pass
                    ...
                    try:
                        ...
                        if "already open" in error_msg.lower() or "is open" in error_msg.lower():
                            if getattr(self, "_recovering_already_open", False):
                                raise Exception("J-Link is already open (recovery in progress)")
                            self._recovering_already_open = True
                            try:
                                self._log_to_gui(QCoreApplication.translate("rtt2uart", "JLink already open detected, force releasing stale session..."))
                                parent = getattr(self.worker, "parent", None)
                                if parent and hasattr(parent, "force_release_matching_sessions"):
                                    try:
                                        parent.force_release_matching_sessions(self._connect_para, exclude_dialog=parent)
                                    except Exception as release_e:
                                        logger.debug(f"force_release_matching_sessions failed: {release_e}")
                                try:
                                ...
                                try:
                                    ...
                                    logger.error(f"Failed to reopen JLink after force release: {error_msg}", exc_info=True)
                                    raise Exception(f"Failed to reopen JLink after force release: {error_msg}")
                            finally:
                ...
                try:
                    ...
                    if connection_check_counter >= connection_check_interval:
                        ...
                        try:
                            ...
                            if not is_conn:
                                ...
                                # 连接丢失时尝试自动重置，失败再停止
                                if self._try_auto_reset_connection("rtt_thread_check_lost"):
                                    time.sleep(0.1)
                                    continue
                                self._auto_stop_on_connection_lost()
                                break  # 退出循环
                            ...
                            if "connection has been lost" in str(e).lower():
                                self._log_to_gui(QCoreApplication.translate("rtt2uart", "JLink connection lost, trying auto reset..."))
                                if self._try_auto_reset_connection("rtt_thread_exception_lost"):
                                    time.sleep(0.1)
                                    continue
                                self._auto_stop_on_connection_lost()
                                break  # 退出循环
                    ...
                    for _ in range(max_read_attempts):
                        try:
                            ...
                            if "connection has been lost" in str(e).lower():
                                self._log_to_gui(QCoreApplication.translate("rtt2uart", "RTT read detected JLink connection lost, trying auto reset..."))
                                if self._try_auto_reset_connection("rtt_read_lost"):
                                    time.sleep(0.1)
                                    continue
                                self._auto_stop_on_connection_lost()
                                return  # 退出整个线程函数
                ...
                try:
                    ...
                    try:
                        ...
                        if ("connection has been lost" in error_str or 
                            ...
                            # 尝试自动重置JLink连接（防重入）
                            if self._try_auto_reset_connection("rtt2uart_read_error"):
                    ...
                    if ("connection has been lost" in error_str or 
                        ...
                        if self._try_auto_reset_connection("rtt2uart_severe_error"):
